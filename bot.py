...
if not TOKEN:
    raise ValueError(
        "❌ TELEGRAM_TOKEN غير موجود في Environment Variables على Render"
    )

if not GROQ_API_KEY:
    raise ValueError(
        "❌ GROQ_API_KEY غير موجود في Environment Variables على Render"
    )
...
def list_books(message):
    ...
    books_list = "\n".join(
        [
            f"📖 {name} ({len(chunks)} صفحة)"
            for name, chunks in library.items()
        ]
    )
...
def delete_book(message):
    ...
    try:
        ...
        book_name = message.text.replace(
            '/delete',
            ''
        ).strip()
...
def handle_document(message):
    ...
    if message.from_user.id != ADMIN_ID:
        ...
        bot.reply_to(
            message,
            "عذراً، رفع الملفات متاح للمشرفين فقط."
        )
    ...
    if not file_name.lower().endswith('.pdf'):
        ...
        bot.reply_to(
            message,
            "الرجاء إرسال ملف بصيغة PDF فقط."
        )
...
def search_relevant_chunks(
    ...
    # إذا لم نجد نتائج، نأخذ بعض الصفحات الأولى
    if not unique_results:

        for book_name, chunks in list(
            library.items()
        )[:2]:
...
4. إذا لم تجد الإجابة في النصوص، قل:
"لم أجد هذه المعلومة في الكتب المتاحة".
            ...
            messages=[
                {
                    "role": "system",
                    "content":
                    "أنت باحث إسلامي دقيق في نقل النصوص من الكتب."
                },
                {
                    "role": "user",
                    "content": prompt
...
def handle_query(message):
    ...
    try:
        ...
        context, sources = search_relevant_chunks(
            query
        )
        ...
        if not context:
            ...
            bot.reply_to(
                message,
                "لم أجد محتوى مناسباً في المكتبة."
            )

            return

        full_text = generate_answer_with_groq(
            query,
            context
        )
...
if __name__ == "__main__":
    ...
    print(
        "🚀 البوت يعمل الآن مع Groq..."
    )

    print(
        f"📚 عدد الكتب المحملة: {len(library)}"
    )
